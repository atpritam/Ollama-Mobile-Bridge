...
async def chat_stream(request: ChatRequest):
    ...
    async def event_generator() -> AsyncIterator[str]:
        try:
            ...
            # Process flow steps
            async for step in ChatService.orchestrate_chat_flow(context):
                if step.action == FlowAction.SEARCH:
                    # Emit search status
                    yield send_sse_event("status", {
                        "stage": f"searching_{step.search_type}",
                    ...
                    # Streaming for direct responses (no search needed)
                    yield send_sse_event("status", {"stage": "generating"})

                    app_logger.info(f"LLM Call #{step.call_number}: Streaming direct response")
